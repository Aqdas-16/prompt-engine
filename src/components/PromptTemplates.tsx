...
const categories = [
  ...
  { id: "user_templates", name: "Your Templates", icon: <User size={16} /> },
];
...
export function PromptTemplates({ onSelect }: { onSelect: (template: string) => void }) {
  ...
  return (
    <div className="w-full mb-6 z-10 relative">
      <div className="flex items-center gap-2 mb-3">
        ...
        <div className="flex gap-2 overflow-x-auto pb-1 custom-scrollbar">
            ...
            <button
                ...
                className={`whitespace-nowrap px-3 py-1.5 rounded-full flex items-center gap-1.5 text-xs font-semibold transition-all shadow-sm border ${
                activeTab === c.id
                    ...
                    : "border-gray-200 dark:border-white/10 bg-white/50 dark:bg-white/5 backdrop-blur-sm text-gray-500 hover:text-gray-800 dark:hover:text-gray-200"
                }`}
            >
                {c.icon} {c.name}
            </button>
      ...
      <div className="flex gap-3 overflow-x-auto pb-2 snap-x custom-scrollbar">
        {activeTab === "user_templates" ? (
          !user ? (
            <div className="text-sm text-gray-500 italic p-3">Login to see your saved templates</div>
          ) : loading ? (
             <div className="text-sm text-gray-500 italic p-3">Loading...</div>
          ) : userTemplates.length === 0 ? (
            <div className="text-sm text-gray-500 italic p-3">No saved templates yet</div>
          ) : (
            userTemplates.map((t) => (
              <div
                key={t.id}
                onClick={() => onSelect(t.content)}
                className="snap-start min-w-[200px] w-[200px] sm:min-w-[240px] p-3 rounded-xl border border-gray-200 dark:border-white/10 bg-white/40 dark:bg-white/5 backdrop-blur-md cursor-pointer hover:-translate-y-1 hover:shadow-md transition-all group"
              >
                <h4 className="text-sm font-semibold mb-1 text-gray-800 dark:text-gray-200 group-hover:text-blue-500 transition-colors line-clamp-1">{t.title}</h4>
